...
def trainer_lidc(args, model, snapshot_path):
    from datasets.dataset_synapse import LIDC_dataset, StaticGenerator
    ...
    # optimizer = optim.SGD(model.parameters(), lr=base_lr, momentum=0.99, weight_decay=0)
    ...
    for epoch_num in iterator:
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            outputs, distributions = model(image_batch, label_batch)
            loss_ce = ce_loss(outputs, label_batch[:].long())
            loss_dice = dice_loss(outputs, label_batch, softmax=True)
            kl_levels = []
            for level, (prior, posterior) in enumerate(distributions):
                kl_loss = kl_divergence(prior, posterior)
                kl_per_batch = torch.mean(kl_loss, dim=[1, 2])
                kl_level = torch.mean(kl_per_batch)
            ...
            writer.add_scalar('info/loss_de', loss_dice, iter_num)
            writer.add_scalar('info/loss_kl', loss_kl, iter_num)
...
def trainer_synapse(args, model, snapshot_path):
    ...
    for epoch_num in iterator:
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            loss_kl = torch.sum(torch.stack(kl_levels))
            # loss = 0.4 * loss_ce + 0.6 * loss_dice
            loss = 0.4 * loss_ce + 0.5 * loss_dice + 0.1 * loss_kl
            optimizer.zero_grad()
            loss.backward()
            # nn.utils.clip_grad_norm(model.parameters(), 1)
            optimizer.step()
