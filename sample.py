...
def inference(args, model, test_save_path=None, is_volume=True):
    ...
    for sampled_batch in testloader:
        ...
        image, label, base_case_name = sampled_batch["image"], sampled_batch["label"], sampled_batch['case_name'][0]
    if test_save_path is not None:
        image_plot = image.squeeze(0).cpu().detach().numpy()
        label_plot = label.squeeze(0).cpu().detach().numpy()
        for level in range(len(image_plot)):
            np.save(os.path.join(test_save_path, f"level{level}_" + base_case_name + "_image.npy"), np.rot90(image_plot.astype(np.float32)[level].squeeze(), k=3)) 
            np.save(os.path.join(test_save_path, f"level{level}_" + base_case_name + "_label.npy"), np.rot90(label_plot.astype(np.float32)[level].squeeze(), k=3)) 
    for i_batch in tqdm(range(sample_num)):
        ...
        logging.info('idx %d case %s mean_dice %f(%f) mean_hd95 %f(%f) mean_iou %f(%f)' % (i_batch, case_name, np.mean(metric_i, axis=0)[0], np.var(metric_i, axis=0)[0], np.mean(metric_i, axis=0)[1], np.var(metric_i, axis=0)[1], np.mean(metric_i, axis=0)[2], np.var(metric_i, axis=0)[2]))
    ...
    var_iou = np.var(metric_list, axis=0)[2]
    logging.info('Testing performance in best val model: mean_dice : %f(%f) mean_hd95 : %f(%f) mean_iou : %f(%f)' % (performance, var_performance, mean_hd95, var_hd95, mean_iou, var_iou))
