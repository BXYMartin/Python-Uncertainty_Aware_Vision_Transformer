...
def inference(args, model, test_save_path=None):
    ...
    for level in range(image.shape[1]):
        ood_image[0,level,:,:] = image[0,level,:,:]
        ood_image[0, level, 256:300, 200:256] = 0.8
        if test_save_path is not None:
            np.save(os.path.join(test_save_path, f"level{level}_" + base_case_name + "_patch.npy"), np.rot90(ood_image[0,level,:,:].astype(np.float32).squeeze(), k=3)) 
